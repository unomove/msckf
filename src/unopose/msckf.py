...
class MSCKF(object):
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        state_cov_fixed = (self.state_server.state_cov + self.state_server.state_cov.T) / 2
    ...
    def state_augmentation(self, time):
        ...
        state_cov = np.zeros((old_rows+6, old_cols+6))
        state_cov[:old_rows, :old_cols] = self.state_server.state_cov
        ...
        # r1 = J @ P11       # Shape (6,21)
        state_cov[old_rows:, :old_cols] = np.concatenate((J @ P11, J @ P12), axis=1)      # Shape (6,27)
        state_cov[:old_rows, old_cols:] = state_cov[old_rows:, :old_cols].T
        state_cov[old_rows:,old_cols:] = J @ P11 @ J.T
    ...
    def prune_cam_state_buffer(self):
        ...
        print ("cam_id", cam_id, rm_cam_state_ids)
        for cam_id in rm_cam_state_ids:
            idx = list(self.state_server.cam_states.keys()).index(cam_id)
            ...
            print ("resize", self.state_server.state_cov.shape)
    ...
    def reset(self):
        ...
        imu_state.timestamp = None
        imu_state.orientation = np.array([0., 0., 0., 1.])
        imu_state.position = np.zeros(3)
        imu_state.velocity = np.zeros(3)
        imu_state.gyro_bias = np.zeros(3)
        imu_state.acc_bias = np.zeros(3)
        imu_state.orientation_null = np.array([0., 0., 0., 1.])
        imu_state.position_null = np.zeros(3)
        imu_state.velocity_null = np.zeros(3)
    ...
    def online_reset(self):
        ...
        print('convariance std: ', max(position_x_std, position_y_std, position_z_std 
            ))
        if max(position_x_std, position_y_std, position_z_std 
            ) < self.config.position_std_threshold:
            return

        print('Start online reset... std: ')
